...
func (c *coreImpl) RegisterTaxonomy(_ context.Context, input TaxonomyInput) (*Taxonomy, error) {
	// UPSERT behavior
	var existing models.Taxonomy
	err := c.db.Where("slug = ?", input.Slug).First(&existing).Error
	...
	if err == nil {
		...
		if len(updates) > 0 {
			if err := c.db.Model(&models.Taxonomy{}).Where("slug = ?", input.Slug).Updates(updates).Error; err != nil {
				return nil, fmt.Errorf("coreapi RegisterTaxonomy update: %w", err)
			}
		}
		c.db.Where("slug = ?", input.Slug).First(&existing)
...
func (c *coreImpl) GetTaxonomy(_ context.Context, slug string) (*Taxonomy, error) {
...
func (c *coreImpl) ListTaxonomies(_ context.Context) ([]*Taxonomy, error) {
	var mList []models.Taxonomy
	if err := c.db.Order("label ASC").Find(&mList).Error; err != nil {
...
func (c *coreImpl) UpdateTaxonomy(_ context.Context, slug string, input TaxonomyInput) (*Taxonomy, error) {
	var m models.Taxonomy
	if err := c.db.Where("slug = ?", slug).First(&m).Error; err != nil {
...
func (c *coreImpl) DeleteTaxonomy(_ context.Context, slug string) error {
	if err := c.db.Where("slug = ?", slug).Delete(&models.Taxonomy{}).Error; err != nil {
...
func (c *coreImpl) ListTerms(_ context.Context, nodeType string, taxonomy string) ([]*TaxonomyTerm, error) {
	var mTerms []models.TaxonomyTerm
	query := c.db.Where("taxonomy = ?", taxonomy)
	if nodeType != "" {
...
func (c *coreImpl) GetTerm(_ context.Context, id uint) (*TaxonomyTerm, error) {
...
func (c *coreImpl) CreateTerm(_ context.Context, term *TaxonomyTerm) (*TaxonomyTerm, error) {
...
func (c *coreImpl) UpdateTerm(_ context.Context, id uint, updates map[string]interface{}) (*TaxonomyTerm, error) {
...
func (c *coreImpl) DeleteTerm(_ context.Context, id uint) error {
	if err := c.db.Delete(&models.TaxonomyTerm{}, id).Error; err != nil {
