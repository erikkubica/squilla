      ...
      <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex h-64 flex-col items-center justify-center gap-3 text-muted-foreground">
            ...
            <p className="text-[15px] font-medium text-muted-foreground">No email rules found</p>
            {rules.length === 0 && (
              <p className="text-[13px] text-muted-foreground">Click &ldquo;Add Rule&rdquo; to get started.</p>
            )}
          </div>
        ) : (
            ...
            <Table className="w-full border-separate border-spacing-0">
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="px-3 py-2.5 bg-muted border-b border-border text-[10.5px] font-semibold uppercase tracking-[0.06em] text-muted-foreground whitespace-nowrap">Action</TableHead>
                  <TableHead className="px-3 py-2.5 bg-muted border-b border-border text-[10.5px] font-semibold uppercase tracking-[0.06em] text-muted-foreground whitespace-nowrap">Node Type</TableHead>
                  <TableHead className="px-3 py-2.5 bg-muted border-b border-border text-[10.5px] font-semibold uppercase tracking-[0.06em] text-muted-foreground whitespace-nowrap">Template</TableHead>
                  <TableHead className="px-3 py-2.5 bg-muted border-b border-border text-[10.5px] font-semibold uppercase tracking-[0.06em] text-muted-foreground whitespace-nowrap">Recipient</TableHead>
                  <TableHead className="px-3 py-2.5 bg-muted border-b border-border text-[10.5px] font-semibold uppercase tracking-[0.06em] text-muted-foreground whitespace-nowrap">Enabled</TableHead>
              ...
              <TableBody>
                {filtered.map((rule) => (
                  <TableRow key={rule.id} className="group bg-card hover:bg-muted">
                    ...
                    <TableCell className="px-3 py-2.5 border-b border-border text-[13px] text-muted-foreground">
                      {getTemplateName(rule.template_id)}
                    </TableCell>
                    <TableCell className="px-3 py-2.5 border-b border-border">
                      <div className="flex items-center gap-2">
                        <Badge className="bg-muted text-foreground hover:bg-muted border-0 text-xs">
                          {rule.recipient_type}
                        </Badge>
                    ...
                    <TableCell className="px-3 py-2.5 border-b border-border">
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 /20 ${rule.enabled ? "bg-primary" : "bg-border"}`}
                      >
                        <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-card transition-transform ${rule.enabled ? "translate-x-[18px]" : "translate-x-[2px]"}`} />
                      </button>
                    </TableCell>
