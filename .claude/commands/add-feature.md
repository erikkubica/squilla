...
This command provides a standardized workflow for implementing features in **Squilla** while adhering to our architecture, performance requirements (sub-50ms TTFB), and "Zero-Rebuild" ethos.
...
3.  **Endpoint Design:** Consult `Squilla API Specification` (api-spec.md) to define new REST routes if an API surface is needed.
...
2.  **Connect the Service:** Register your new service in `cmd/squilla/main.go` to ensure lifecycle management (init, health checks).
3.  **Test:** Create a test file in the relevant `internal/` sub-package. Squilla prioritizes unit tests for Core CMS Logic and Integration handlers.
    *   `go test ./internal/...`
4.  **Verify:**
...
### Execution
Proceed by creating your implementation files, running `go mod tidy` if new dependencies were added, and verifying against the `cmd/squilla/main.go` entry point.